...
void PairAllegro::compute(int eflag, int vflag){
  ...
  std::vector<int> neigh_per_atom(inum, 0); //dosedah nlocal kar je 324 prostorov

#pragma omp parallel for reduction(+:nedges)
  for(int ii = 0; ii < inum; ii++){ //do sedaj nlocal kar je do 323 sedaj samo 322
    int i = ilist[ii];
    int itag = tag[i]; //dodala
    int itype = type[i]; //dodala
    ...
    for(int jj = 0; jj < jnum; jj++){
      ...
      if(rsq <= cutoff*cutoff) {
        neigh_per_atom[ii]++;//ok zaporedno napise stevila sosedov od 0 do 322 vmes preskoci index od P tako da je notri 322 ionov 
        nedges++;
      }
    }
  }
  ...
  std::vector<int> cumsum_neigh_per_atom(inum); //dodala

  for(int ii = 1; ii < inum; ii++){ //dodala
    cumsum_neigh_per_atom[ii] = cumsum_neigh_per_atom[ii-1] + neigh_per_atom[ii-1];
  }

  torch::Tensor pos_tensor = torch::zeros({inum, 3}); //iz ntotal na inum 
  ...
  torch::Tensor ij2type_tensor = torch::zeros({inum}, torch::TensorOptions().dtype(torch::kInt64));
...
#pragma omp parallel for
  for(int ii = 0; ii < inum; ii++){
    ...
    ij2type[itag-1] = type_mapper[itype]; //= type_mapper[itype - 1];
    ...
    for(int jj = 0; jj < jnum; jj++){
     std::cout<<" in j loop "<<std::endl;
      int j = jlist[jj];
      j &= NEIGHMASK;
     //if(j >= nlocal){continue;} //j je index normalen preslikan iz indexa jj ki je pa za stevilo sosedov j jlisu danega centralnega iona i
      int jtag = tag[j];
      int jtype = type[j];
      if (itype == 3) {
      std::cout<<"type bead "<<std::endl;
      ...
      edges[0][edge_counter] = itag-1;
      edges[1][edge_counter] = jtag-1;

  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        //std::cout<<" forces in lj wolf "<< " x "<<f[i][0] << " fy "<<f[i][1] << " z "<<f[i][2] <<  " type "<<itype <<" tag "<<tag[i]<<std::endl; 
    ...
    std::cout << " x wolf"<<f[i][0] <<  " fy  wolf"<<f[i][1] << " z  wolf"<<f[i][2] <<  " type  wolf"<<itype <<" tag  wolf "<<tag[i]<< " i  wolf"<< i <<std::endl;
  }
...
{ std::cout<< " in settings "<<std::endl;
  ...
  if (narg == 3) cut_coul = utils::numeric(FLERR,arg[2],false,lmp);
...
{ //std::cout<< " in init stayle "<<std::endl;
  if (!atom->q_flag)
...
{ //std::cout<< " in init one "<<" i "<<i <<" j "<<j<<std::endl;
  if (setflag[i][j] == 0) {
...
{ std::cout<< " in write_restart "<<std::endl;
...
{ std::cout<< " in read_restart "<<std::endl;
...
{ std::cout<< " in write_restart settings "<<std::endl;
...
{ std::cout<< " in read _restart settings"<<std::endl;
  int me = comm->me;
  if (me == 0) {
...
{ std::cout<< " in write_data "<<std::endl;
...
{std::cout<< " in write_data all"<<std::endl;
  for (int i = 1; i <= atom->ntypes; i++)

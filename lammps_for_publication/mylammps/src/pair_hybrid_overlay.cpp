...
void PairHybridOverlay::coeff(int narg, char **arg)
{
  ...
  for (m = 0; m < nstyles; m++) {
    ...
    if (strcmp(arg[2],keywords[m]) == 0) {
      //std::cout << "strcmp(arg[2],keywords[m]) "<< (arg[2])<<std::endl;
      if (multiple[m]) { //to ni ker ni isti veckrat uporabljen
        multflag = 1;
        //std::cout<<" in multiple "<< arg[3] <<std::endl;
  ...
  //std::cout << "strcmp(arg[2],keywords[m]) "<< (arg[2])<<std::endl; // pove tipe ki jih uposteva allegro ** ljcut coul vsakefa posebej
  arg[1+multflag] = arg[0];
 //std::cout << "strcmp(arg[1],keywords[m]) "<< (arg[1])<<std::endl;
  ...
  for (int i = ilo; i <= ihi; i++) {
    for (int j = MAX(jlo,i); j <= jhi; j++) {
      if (none) {
        ...
        //std::cout<< " types " << i << " "<< j << " etflag[i][j] " << styles[m]->setflag[i][j] <<std::endl;
        int k;
        for (k = 0; k < nmap[i][j]; k++)
          if (map[i][j][k] == m) break;
        if (k == nmap[i][j]) map[i][j][nmap[i][j]++] = m; /*{std::cout<< "2 k == nmap[i][j] " << k << "nmap[i][j] "<<nmap[i][j] <<" map[i][j][nmap[i][j]++] "<<map[i][j][nmap[i][j]++]<< " m "<<m<<std::endl;
            map[i][j][nmap[i][j]++] = m*/
        setflag[i][j] = 1;
        count++;
      }

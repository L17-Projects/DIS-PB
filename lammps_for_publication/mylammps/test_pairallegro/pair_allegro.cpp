...
//setflag bran iz coeff ki mora vedno biti za vse ker mora biti ceof **, prebere jih v pair hybride overlay so za vse,
 ...
 /* type_mapper.resize(ntypes, -1);
  ...
  std::cout<<type_mapper[0]<<" type_mapper[0] "<<std::endl;// ntapyes, -1  je ta 1 ce je 1 
  std::cout<<type_mapper[1]<<" type_mapper[1] "<<std::endl;// ntapyes, -1  je ta 0
  std::cout<<type_mapper[2]<<" type_mapper[2] "<<std::endl; // ce je ntapyes, -1 ti ta ostane -1
  // set setflag i,j for type pairs where both are mapped to elements
   for (int i = 1; i <= ntypes; i++) { // od 1 2 3 ker ntaypes = 3 
    for (int j = i; j <= ntypes; j++) { // j 1 2 3, prvi for je 1 1, 1 2, 1 3, 2 2, 2 3, 3 3
        if ((type_mapper[i-1] >= 0) && (type_mapper[j-1] >= 0)) {
            setflag[i][j] = 1;
        //if type mappr[0] vecji ali enak od 0 (defoult -1 ) in type mapper [0] vecji ali enak 0 potem das setflag da je 1 

        std::cout<<"typemapper i "<<type_mapper[i-1]<<" typemapper j "<<type_mapper[j-1]<< " setflag[i][j] "<<setflag[i][j]<<std::endl;}}}

...
{ std::cout<<" in coeff overlay "<<std::endl;
  ...
  for (m = 0; m < nstyles; m++) {
    ...
    if (strcmp(arg[2],keywords[m]) == 0) {
       std::cout << "keywords[m]) "<< keywords[m]<<std::endl;
      if (multiple[m]) {
        exit(EXIT_FAILURE);
        std::cout << "in multiple  "<< m<<std::endl;
        multflag = 1;
  ...
  for (int i = ilo; i <= ihi; i++) {
    for (int j = MAX(jlo,i); j <= jhi; j++) {
      if (none) {
        ...
        int k;
        std::cout<<" nmap[i][j] "<<nmap[i][j]<<" i "<<i<<" j "<<j<< std::endl;
      ...
      std::cout<<" nmap[i][j] "<<nmap[i][j]<<" i "<<i<<" j "<<j<<" map[i][j][nmap[i][j]++] "<<map[i][j][nmap[i][j]++]<<" m "<<m<<std::endl;
    }
  }
...
{ std::cout<<" in init_svector overlay "<<std::endl;
...
{ std::cout<<" copy_svector overlay "<<std::endl;
  int n=0;
